...
Vous pouvez ajouter des liens externes dans votre chatbot en utilisant la syntaxe Markdown habituelle pour les liens, mais si vous voulez qu'un bouton de réponse mène vers un lien externe, il faut préfixer ce lien avec `link:`

```markdown
1. [Bouton de réponse ordinaire : lien vers une réponse du chatbot](#titre de la réponse)
2. [Lien vers une URL externe](link:https://exemple.fr)
```

Les liens externes sont affichés dans un nouvel onglet. Les boutons de réponse avec des liens externes ont une icône de lien pour les différencier des boutons ordinaires qui mènent vers des réponses du chatbot.
...
On peut aussi utiliser la balise HTML `<audio>` pour intégrer un lecteur audio dans le message.

```html
<audio controls>
  <source src="URLduFichierAudio">
</audio>
```
...
- Les variables sans préfixe `_` sont interprétées au moment de l'affichage d'un message : si on utilise de l'aléatoire, on pourra avoir une sélection différente à chaque affichage du même message
- Les variables avec préfixe `_` sont interprétées au tout début, avant même l'affichage du chatbot, ce qui permet d'utiliser des variables fixes qui contiennent des options en fin de message, des déclencheurs, etc.
